...
out_path = "/data/sm979441/MF_results/liana"
data = sc.read_h5ad("/data/Schneider_lab/biopsy_MPN_CML/MF/data/pipeline_output/saved_objects/MF_subcluster_subset.h5ad")
data = data[(data.obs["treatment"] == "Control")  |  (data.obs["treatment"] == "MF_pre_treatment")]
data.raw = data

for i in set(data.obs['stage']):
    print(i)
    lr = li.method.cellphonedb(data[data.obs['stage'] == i],
                               groupby='subcluster',
                               expr_prop=0.1,
                               verbose=True,
